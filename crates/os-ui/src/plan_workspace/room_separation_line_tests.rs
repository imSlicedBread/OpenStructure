...
fn two_click_preview_does_not_mutate_then_creates_one_line_at_both_dpis() {
    for (size, scale) in PROFILES {
        ...
        h.app.begin_room_separation_line();
        h.frame(vec![]);
        ...
        assert!(h.app.plans.room_separation_line_draft.is_some());
        h.click(h.point(Point2::new(1.0, 0.0)));
        h.settle();
        assert_eq!(h.app.editor.document.model().room_separation_lines.len(), 1);
...
fn escape_cancels_unfinished_two_click_draft_at_both_dpis() {
    for (size, scale) in PROFILES {
        ...
        h.frame(vec![escape()]);
        assert!(
            h.app.plans.room_separation_line_draft.is_none(),
...
fn selected_handle_body_and_blank_canvas_have_distinct_pointer_owners() {
    for (size, scale) in PROFILES {
        let mut h = Harness::new(size, scale);
        let line = os_model::RoomSeparationLine::new(
            "core.room_separation_line",
            os_model::RoomSeparationLineParams {
        ...
        h.app
            .editor
            .command("Fixture", Command::AddRoomSeparationLine(line))
            .unwrap();
        h.settle();
        ...
        assert_eq!(
            h.app.editor.document.model().room_separation_lines[&id].id(),
            id
        );
        let after_handle = h.app.editor.document.model().clone();
        let edited = &after_handle.room_separation_lines[&id].parameters;
        ...
        let end = after_body.room_separation_lines[&id].parameters.end;
        ...
        let pan_start = canvas.center() + egui::vec2(100.0, 0.0);
        let pan_end = pan_start + egui::vec2(30.0, -20.0);
        assert!(canvas.contains(pan_start) && canvas.contains(pan_end));
        h.drag(pan_start, pan_end);
        assert_ne!(
            h.app.plans.cameras[&h.view].center, center,
...
fn collapsing_release_and_stale_revision_do_not_commit_a_drag() {
    for (size, scale) in PROFILES {
        ...
        assert!(h.app.plans.room_separation_line_draft.is_some());
        let external = os_model::RoomSeparationLine::new(
            "core.room_separation_line",
            os_model::RoomSeparationLineParams {
                level: h.app.active_level,
        ...
        h.app
            .editor
            .command("External edit", Command::AddRoomSeparationLine(external))
            .unwrap();
        ...
        assert!(h.app.plans.room_separation_line_draft.is_none());
...
#[test]
fn changing_plan_view_cancels_separator_draft_without_model_mutation() {
    for (size, scale) in PROFILES {
        let mut h = Harness::new(size, scale);
        let second = h
            .app
            .editor
            .create_floor_plan("Second separator plan", h.app.active_level)
            .unwrap();
        h.settle();
